...
def main():
    ...
    for i in range(0, len(ids)):
		...
		t1img=sitk.GetArrayFromImage(imgOrg)
		imgOrg=sitk.ReadImage(dataT2fn)
		t2img=sitk.GetArrayFromImage(imgOrg)
# 		mu=np.mean(mrimg)
# 		maxV=np.max(mrimg)
#  		minV=np.min(mrimg)
# 		print mrimg.dtype
...
#  		mrimg=(mrimg-mu)/(maxV-minV)
		...
		labelimg[labelimg>200]=3 
		labelimg[labelimg>100]=2
		labelimg[labelimg>4]=1
    	#you can do what you want here for for your label img
    
		fileID='%d'%myid
		rate=1
		matOut,matSeg=cropCubic(t1img,t2img,labelimg,fileID,dSeg,step,rate)
		dr1=dice(matOut,matSeg,1)
		dr2=dice(matOut,matSeg,2)
		dr3=dice(matOut,matSeg,3)
		print 'dr1: %d'%dr1,'dr2: %d',dr2,'dr3: %d',dr3
